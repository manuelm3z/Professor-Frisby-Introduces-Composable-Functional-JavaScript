/**
 * Lesson 9: A curated collection of Monoids and their uses
 */
const Sum = x =>
({
    x,
...
const find = (xs, f) =>
    List(xs).foldMap(x => 
        First(f(x) ? Right(x) : Left()), First.empty()).fold(x => x)

find([3, 4, 5, 5, 6, 7], x => x > 4)
//Right(5)
...
const both = Fn(compose(Any, hasVowels)).concat(Fn(compose(Any, longWord)))
